...
const s3 = new AWS.S3({accessKeyId: prjConfig.AWS.AccessKey, secretAccessKey: prjConfig.AWS.SecretKey});
    ...
    try {
        ...
        if (findEmail ?. length != 0) {
            if (userInfo ?. social_media_flag == true) {
                ...
                res.cookie("token", token, {httpOnly: true}).send({status: 200, result: STATUS.SUCCESS, data: data_social, message: msg});
            } else {
                if (findEmail[0] ?. is_verified === true) {
                    ...
                    const deleteOld = await Models.otp.deleteOne({user_email: userInfo.email, otp_action: "Consumer Register"}).exec();
                    ...
                    template = Fs.readFileSync("./lib/register.html", {encoding: "utf-8"});
            ...
            if (userInfo ?. is_admin === 0 && userInfo ?. social_media_flag == false) {
                ...
                var template = tpl.fetch(__dirname + "/../../system/template/userRegistration.tpl");
...
const verifyOtp = async function (data) {
    try {
        ...
        if (otpMatch === true && otpInfo ?. otp_action === "Consumer Register") {
            ...
            const result = await Models.otp.deleteOne({
                otp: otpInfo ?. otp
            }).exec();
            console.log(result);
            let loginObj = {
            ...
            let updatemodule = await Models.user.updateOne(filterModule, moduleUpdate).exec();
            ...
            console.log(JSON.stringify(questionnaireScore, null, 4) + "questionnaire score");
        ...
        } else if (otpMatch === true && otpInfo ?. otp_action === "Admin forgot otp") {
            const result = await Models.otp.deleteOne({
                otp: otpInfo ?. otp
            }).exec();
            data.response = {
...
const login = async function (data) {
    try {
        ...
        var user_data = await Models.user.findOne({email: data.email, deleted_date: null}).exec();
        ...
        if (user_data !== null) {
            if (user_data.usertype_in == true && data.password == undefined) {
                ...
                return data;
            } else if (user_data.usertype_in == false && data.password !== undefined) {
                data.response = {
                    status: 0,
                    result: STATUS.ERROR,
                    message: "User is not valid."
            ...
            if (user_data.is_verified == true && user_data.is_active == true) {
                if (data.password !== undefined) { // Admin Login

                    var login_data = await Models.login.findOne({email: user_data.email}).exec()
                    ...
                    var otp_data = await Models.otp.findOne({user_email: user_data.email}).exec()
                    ...
                    var template = tpl.fetch(__dirname + "/../../system/template/userLogin.tpl");
            ...
            userLogger.info(__filename, 'Login process response ---->  ,' + data.response);
...
const forgot_password = async function (data) {
    try {
        ...
        userLogger.info(__filename, 'forgot_password process request ---->  ,' + data);
        ...
        if (user_data == null) {
            ...
            if ((user_data.is_verified === true || user_data.is_verified == "true") && (user_data.is_active === true || user_data.is_active == "true")) {
                var otp_data = await Models.otp.findOne({user_email: user_data.email}).exec()
                ...
                var template = tpl.fetch(__dirname + "/../../system/template/forgot_password.tpl");
            ...
            userLogger.info(__filename, 'forgot_password process response ---->  ,' + data.response);
            return data;
        }
...
const update_forgot_password = async function (data) {
    try {
        ...
        if (data.password !== data.confirm_password) {
            ...
            userLogger.info(__filename, 'update_forgot_password process request ---->  ,' + data);
            ...
            if (user_data == null) {
                ...
                userLogger.info(__filename, 'update_forgot_password process response ---->  ,' + data.response);
            }
        }
        ...
        userLogger.info(__filename, 'update_forgot_password catch block ---->  ,' + error);
...
const resend_otp = async function (data) {
    try {
        ...
        if (data.otp_action == "Consumer Register") {
            ...
            userLogger.info(__filename, 'Resend_otp process response ---->  ,' + data.response);
            return data;
        }
        ...
        if (data.otp_action !== "Consumer Login" || data.otp_action !== "Consumer Register") {
            data.response = {
                status: 0,
                result: STATUS.ERROR,
    ...
    try {
        ...
        if (user_data !== null) {
            if (user_data.usertype_in == false) {
                ...
                if (user_data.personal_brand !== undefined && user_data.personal_brand !== null) {
                    ...
                    if (user_data.personal_brand.interest !== null) {
                        user_data.personal_brand.interest = user_data.personal_brand.interest.filter(item => item.trim() !== '');
                        interest = input * user_data.personal_brand.interest.length
                    }

                    if (user_data.personal_brand.values !== null) {
                        user_data.personal_brand.values = user_data.personal_brand.values.filter(item => item.trim() !== '');
                        values = input * user_data.personal_brand.values.length
                    }

                    if (user_data.personal_brand.traits !== null) {
                        user_data.personal_brand.traits = user_data.personal_brand.traits.filter(item => item.trim() !== '');
                        traits = input * user_data.personal_brand.traits.length
                    }

                    if (user_data.personal_brand.models !== null) {
                        user_data.personal_brand.models = user_data.personal_brand.models.filter(item => item.trim() !== '');
                ...
                if (user_data.professional_details !== undefined && user_data.professional_details !== null) {
                    var perfessional = module_percentage / 3
                    var professional_count = 0

                    if (user_data.professional_details.mission !== undefined && user_data.professional_details.mission !== null) {
                        professional_count = professional_count + 1
                    }

                    if (user_data.professional_details.vision !== undefined && user_data.professional_details.vision !== null) {
                        professional_count = professional_count + 1
                    }

                    if (user_data.professional_details.education !== undefined && user_data.professional_details.education !== null) {
                        professional_count = professional_count + 1
                    }
            ...
            userLogger.info(__filename, 'User_profile process response ---->  ,' + data);
            return data;
        } else {
            var resp = {
                status: 0,
...
const mobile_user_list = async function (data, authData) {
    try {
        ...
        userLogger.info(__filename, 'mobile_user_list process request ---->  ,' + data);
        ...
        var total_mobile_user = await Models.user.find({usertype_in: data.usertype_in, deleted_date: data.deleted_date}).exec()
        ...
        if (Object.keys(filter).length !== 0) {
            ...
            var user_list = await Models.user.aggregate([
                {
                    '$match': data
                },
                {
            ...
            var total = await Models.user.aggregate([
                {
                    '$match': data
                },
                {
        ...
        userLogger.info(__filename, 'mobile_user_list process response ---->  ,' + data);

        return data;
    } catch (error) {
        userLogger.info(__filename, 'mobile_user_list catch block ---->  ,' + error);
...
const admin_user_list = async function (data, authData) {
    try {
        ...
        userLogger.info(__filename, 'admin_user_list process request ---->  ,' + data);
        ...
        var total_admin_user = await Models.user.find({usertype_in: data.usertype_in, deleted_date: data.deleted_date}).exec()
...
const createAdminUser = async function (data, authData) {
    ...
    if(data.user_id == undefined) {
        ...
        let server_url = prjConfig.FrontendForgotPassword.url + Buffer.from(data.email).toString('base64');
        
        var template = tpl.fetch(__dirname + "/../../system/template/admin_registration.tpl");
        ...
        data.response = {
            ...
            result: STATUS.SUCCESS,
            data: savedAdmin,
            message: "Email has been sent to the respective email id for setting up the password."
        }
        return data;
    } else {
        ...
        var savedAdmin = await Models.user.findOneAndUpdate(filter, {
            $set: data
        }, {new: true}).lean();
        var user_data = await Models.user.findOne({user_id: data.user_id, deleted_date: null}).exec();
        var getRole = await Models.role.findOne({role_id: user_data.role_id});
...
const changeAdminPassword = async function (data, authData) {
    ...
    let updateLogin = await Models.login.findOneAndUpdate(filter, {
        $set: update
    }, {new: true});
    let updateUser = await Models.user.findOneAndUpdate(filter, {
        $set: {
            is_verified: true
        }
    }, {new: true});
    ...
    try {
        ...
        switch (data.timeFilter) {
            ...
            case 'yearly': 
                ...
                break;
            case 'half-yearly':
                if (currentMonth < 6) { // First half: January to June
                    startDate = new Date(Date.UTC(currentYear, 0, 1)); // January 1st
        ...
        // Temp file path
        const filePath = path.join(__dirname, `/excel/output_${
            Date.now()
        }.xlsx`);
        ...
        console.log(e)
        userLogger.info(__filename, 'mobile_user_list process response ---->  ,' + e);
    }
}
  ...
  try {
    ...
    if (user_data !== null) {
      ...
      if (payment_data.length == 0) {
        var subscription_data = await Models.subscription.findOne({payment_type: "Free"}).exec()
      }else{
        var subscription_data = await Models.subscription.findOne({_id: payment_data[0].subscription_id}).exec()
      }
...
const social_media_account = async (req) => {
  try {
    ...
    var user_data = await Models.user.findOne({email: userInfo.email, deleted_date: null}).exec()
...
const user_mobile_update = async function (data, authData) {
  try {
    ...
    var user_data = await Models.user.findOne({mobile_no: data.mobile_no, deleted_date: null}).exec()
...
const user_app_mobile_signup = async function (data, authData) {
  try {
    ...
    var user_email_data = await Models.user.findOne({email: data.email, deleted_date: null, usertype_in: false}).exec()
    var user_mobile_data = await Models.user.findOne({mobile_no: data.mobile_no, deleted_date: null, usertype_in: false}).exec()
...
const user_app_mobile_login = async function (data, authData) {
  try {
    ...
    var user_data = await Models.user.findOne({mobile_no: data.mobile_no, deleted_date: null, usertype_in: false}).exec()
