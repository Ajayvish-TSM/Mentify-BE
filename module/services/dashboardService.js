const dashboardData = async (data, authData) => {
    try {
        ...
        if (timeframe === 'monthly') {
            ...
            if (currentMonth < 6) { // First half of the year
              firstDayOfHalfYear = new Date(currentDate.getFullYear(), 0, 1);
              lastDayOfHalfYear = new Date(currentDate.getFullYear(), 5, 30);
              halfYearName = `${new Date(firstDayOfHalfYear).toLocaleString('default', { month: 'short' })}-${new Date(lastDayOfHalfYear).toLocaleString('default', { month: 'short' })}`;
        ...
        const eventCount = await Models.event.aggregate(
          [
            ...
            {
              $addFields: {
                start: { $dateToString: { format: "%Y-%m-%dT%H:%M:%S", date: "$start_date_converted" } },
                eend: { $dateToString: { format: "%Y-%m-%dT%H:%M:%S", date: "$start_date_converted" } },
              },
            },
  					{
              $addFields: {
                estart: { $dateToString: { format: "%Y-%m-%dT%H:%M:%S", date: "$end_date_converted" } },
                end: { $dateToString: { format: "%Y-%m-%dT%H:%M:%S", date: "$end_date_converted" } },
              },
            },
            {
              $project: {
        ...
        if (refund_event_amount.length == 0 || refund_event_amount[0].refund_event_amount == null) {
          var total_refund_event_amount = 0
        }else{
          var total_refund_event_amount = refund_event_amount[0].refund_event_amount
        }

        if (eventData[0]?.event_amount !== undefined && eventData[0]?.event_amount !== null) {
          total_event_amount = eventData[0]?.event_amount - total_refund_event_amount
        }

        if (courseData[0]?.course_amount !== undefined && courseData[0]?.course_amount !== null) {
          total_course_amount = courseData[0]?.course_amount
        }

        if (subscriptionData[0]?.subscription_amount !== undefined && subscriptionData[0]?.subscription_amount !== null) {
          total_subscription_amount = subscriptionData[0]?.subscription_amount
        }

        var total_revenue = total_event_amount + total_course_amount + total_subscription_amount
...
const convertToCsv = async (data, timeframe) => {
    let csv = '';
        ...
        if (timeframe === 'monthly') {
            ...
            // Add month header
            csv += `${new Date(monthData[0].start_date).toLocaleString('default', { month: 'long' })}\n`;
            ...
            groupedByQuarter.forEach((quarterData, index) => {
                ...
                const quarterIndex = Math.floor(new Date(quarterData[0].start_date).getMonth() / 3) + 1;
                console.log(quarterIndex, 'quarterIndex')

                // Determine the time period based on the quarter index
                const timePeriod = quarterIndex === 1 ? "Jan-Mar" :
                                quarterIndex === 2 ? "Apr-Jun" :
                                quarterIndex === 3 ? "Jul-Sep" : "Oct-Dec";
...
const exportToCsv = async (data, authData) => {
    ...
    function groupDataByHalfYear(data) {
        ...
        data.forEach((item) => {
          ...
          if (currentMonth === -1 || Math.floor(currentMonth / 6) === Math.floor(month / 6)) {
      ...
      function groupDataByQuarter(data) {
        ...
        data.forEach((item) => {
          ...
          if (currentMonth === -1 || Math.floor(currentMonth / 3) === Math.floor(month / 3)) {
