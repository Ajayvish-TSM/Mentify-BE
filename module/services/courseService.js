...
const create = async function (data, authData) {
  try {
    ...
    if (data?.course?.courseedition_id !== undefined && data?.course?.courseedition_id !== null) {
    ...
    const category = await Models.category.findOne({ category_id: CourseInfo.category_id }).exec();
    CourseInfo.category_name = category?.category_name;

    const isCourse = await Models.course.findOne({
    ...
    const isCourseEdition = await Models.courseEdition.findOne({
      author_name: CourseInfo?.author_name,
      category_id: CourseInfo?.category_id,
      course_title: CourseInfo?.course_title,
      course_type: CourseInfo?.course_type,
    ...
    if (isCourse || isCourseEdition) {
      ...
      if (CourseInfo['status'] != 4) {
        ...
        try {
          ...
          const logData = {
            'course_id': (savedEdition?.course_id) ? (savedEdition?.course_id) : (savedEdition?.courseedition_id),
            'action': 'Initiated'
          };
...
const createContent = async function (data, authData) {
  try {
    ...
    if (data?.chapter?.chapter_id != undefined && data?.chapter?.chapter_id != "") {
      const filter = {
      ...
      if (data?.chapter?.is_deleted != undefined && data?.chapter?.is_deleted == true) {
    ...
    for (var i = 0; i < ModuleInfo.length; i++) {
      if (ModuleInfo[i]['_id'] != undefined && ModuleInfo[i]['_id'] != "") {
        ...
        if (ModuleInfo[i]['is_deleted'] != undefined && ModuleInfo[i]['is_deleted'] == true) {
          moduleUpdate.deleted_date = new Date();
          moduleUpdate.deleted_by = decoded?._id;
        }
        let updatemodule = await Models.module.updateOne(filterModule, moduleUpdate).exec();
        if (!updatemodule) {
    ...
    if (data?.status == 4 && data?.flag == "moderator") {
      ...
      try {
        if (data.is_active != undefined) {
          ...
          let oldcourse_data = await Models.course.findOne({ course_id: data?.course_id }).exec();
          ...
          let CourseInfo = await Models.courseEdition.findOne({ courseedition_id: data.course_id }).exec()
          ...
          for (var i = 0; i < chapterEdition_data.length; i++) {
            ...
            let chapter_data = await Models.chapter.findOne({ chapterEdition_id: chapterEdition_data[i].chapterEdition_id }).exec()
            let savedChapter;
            ...
            for (var j = 0; j < moduleEdition_data.length; j++) {
              ...
              let module_data = await Models.module.findOne({ moduleEdition_id: moduleEdition_data[j].moduleEdition_id }).exec()
              let savedModule;
      ...
      var course_data = await Models.courseEdition.findOne({ courseedition_id: data?.course_id }).exec()
      if (course_data !== null && course_data.status !== undefined && course_data.status == 2) {
        data.status = 3
      }
      let updatecourseedition = await Models.courseEdition.findOneAndUpdate({
...
const addCourseAssessment = async function (data, authData) {
  try {
    ...
    if (question?.assignment_id != undefined && question?.assignment_id != "") {
      ...
      let updateassignment = await Models.assignment.findOneAndUpdate(filter, { $set: question }, { new: true });
      if (!updateassignment) {
        console.error('Error updating document:', updateErr);
      } else {
        data.response = {
...
const getChapterList = async function (data, authData) {
  try {
    ...
    if (data.flag == null) {
      ...
      // console.log("    getChapterList     null   --------->   ", data.flag)
      let record = await Models.course.findOne({ course_id: data.course_id }).exec()
...
const getCourseDetails = async function (data, authData) {
  try {
    ...
    if (data.flag == null) {
      ...
      var courseInfo = await Models.courseEdition.aggregate([
        {
          $match: {
            courseedition_id: data?.course_id,
            deleted_date: null
      ...
      var courseInfo = await Models.course.aggregate([
        {
          $match: {
            course_id: data?.course_id,
            deleted_date: null
        ...
        },
        {
          ...
          $project: {
            ...
            declined_user: 1,
            community: "$communityInfo",
            razorpay_payment_status: "$payment_detail_info.razorpay_payment_status",
            categoryName: "$categoryInfo.category_name"
          }
        }
      ]).exec();
      ...
      if (decoded.usertype_in == false) {
        ...
        if (courseInfo[0]?.declined_user !== undefined && courseInfo[0]?.declined_user !== null && courseInfo[0]?.declined_user.includes(user_data?.user_id)) {
          courseInfo[0].block_button = true
        } else {
          courseInfo[0].block_button = false
        }

        var feedback_data = await Models.feedback.findOne({ user_id: data.user_id, course_id: data.course_id, feed_type: "feedback" }).exec()
        courseInfo[0].feedback_data = feedback_data
        console.log("courseInfo[0].feedback_data  ------->   ", courseInfo[0].feedback_data)
      }
...
const deleteModule = async function (data, authData) {
  try {
    ...
    let updatemodule = await Models.moduleEdition.updateOne(filterModule, data).exec();
...
const myLearningSave = async function (data, authData) {
  try {
    ...
    if (data.completed_flag == true) {
      const addCompletedLearning = new Models.myCompletedLearning(data.learning);
...
const createContentEdition = async function (data, authData) {
  try {
    ...
    if (chapterData._id == undefined || chapterData._id == "") {
      ...
      if (chapterData?.is_deleted != undefined && chapterData?.is_deleted == true) {
    ...
    if (ModuleInfo.length > 0) {
      for (var i = 0; i < ModuleInfo.length; i++) {
        if (ModuleInfo[i]['_id'] != undefined && ModuleInfo[i]['_id'] != "") {
          ...
          let updatemodule = await Models.moduleEdition.updateOne(filterModule, moduleUpdate).exec();
        } else {
          delete ModuleInfo[i]['module_id'];
          delete ModuleInfo[i]['_id'];
          let module = {
...
const continue_course_Add = async function (data, authData) {
  try {
    ...
    if (data.course_id == undefined || data.chapter_id == undefined || data.module_id == undefined || data.user_id == undefined || data.end_time.length == 0 || data.status.length == 0) {
        data.response = {
          status: 0,
          result: STATUS.ERROR,
    ...
    let module_data = await Models.continue_course.findOne({ module_id: data.module_id, user_id: data.user_id }).exec()
  ...
  try {
    ...
    if (data.status == "Continue") {
      ...
      for (var i = 0; i < record.length; i++) {
        var course_data = await Models.course.findOne({ course_id: record[i]._id }).exec()

        var module_data = await Models.module.find({ course_id: course_data.courseedition_id }).exec()
        var countinue_cou_data = await Models.continue_course.find({ course_id: record[i]._id, status: "Completed", user_id: data.user_id }).exec()

        console.log("module_data.length              ------>  ", module_data.length)
        console.log("countinue_cou_data.length       ------>  ", countinue_cou_data.length)

        var certificate_data = await Models.certificate.findOne({ course_id: record[i]._id, user_id: data.user_id }).exec()
    ...
    if (data.status == "Completed") {
      ...
      let module_data = await Models.module.find({ course_id: course_data.courseedition_id }).exec()
      let certificate_data = await Models.certificate.findOne({ user_id: data.user_id, course_id: data.course_id }).exec()
