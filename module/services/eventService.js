        ...
        '$cond': {
          'if': {
            '$and': [
              {
                '$lte': [
                  '$start_date', todays_date.toISOString()
                ]
              }, {
                '$gt': [
                  '$end_date', todays_date.toISOString()
                ]
              }
            ]
          },
      ...
      var record = await Models.event.aggregate([
        ...
        },
        {
          '$addFields': {
            'sold_ticket_amount': {
              '$multiply': [
                '$amount', '$sold_ticket_count'
              ]
            }, 
            'remaining_ticket_count': {
              '$subtract': [
                '$seat', '$sold_ticket_count'
              ]
            }
          }
        }
      ...
      var record = await Models.event.aggregate([
        ...
        },
        {
          '$addFields': {
            'sold_ticket_amount': {
              '$multiply': [
                '$amount', '$user_event_attend_count'
              ]
            }, 
            'remaining_ticket_count': {
              '$subtract': [
                '$seat', '$user_event_attend_count'
              ]
            }
          }
        },
