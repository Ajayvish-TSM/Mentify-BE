...
const { DEFAULT_USER_IMAGE, ROLES, USER_STATUS, ACTIVATED_BY, STATUS } = require("./constants");
...
router.post('/forgotpassword', (req, res) => {
    ...
    userLogger.info(__filename, 'forgot password payload data => ' + { data: `${JSON.stringify(postData)}` });
    ...
    const promise = new Promise(async function (resolve, reject) {
        try {
            let rows = await mysql.getSelectQueryData("SELECT * FROM qq_user where email = ?", [postData.email], pool);
            if (Object.keys(rows).length === 0) {
                userLogger.error(__filename, `Return null SELECT * FROM qq_user where email = ${postData.email}`);
                ...
                console.log("token", { name: config.MAIL.SENDER_NAME, email: config.MAIL.SENDER_EMAIL });
                ...
                resolve([{ status: 1, message: "Reset password link has beed sent..." }]);
            }
        } catch (error) {
    ...
    promise.then(async function (data) {
        userLogger.info(__filename, 'User Data ' + { data: `${JSON.stringify(data)}` });
        res.status(200).json({ "status": 1, "message": "Forgot password send success...! ", "response": data });
    }).catch(async function (error) {
        userLogger.info(__filename, 'Forgot password send error' + { data: `${JSON.stringify(error)}` });
...
router.post('/login', (req, res) => {
    ...
    userLogger.info(__filename, `Login payload data => ${JSON.stringify(postData)}`);
    const user = {
        "email": postData.email
    }
    // const mysql = new mysqli();
    ...
    promise.then(async function (data) {
        ...
        res.status(200).json({ "status": 1, "message": "Login success...! ", "response": response });
    }).catch(async function (error) {
...
router.post('/addshop', (req, res) => {
    const postData = req.body;
    //userLogger.info(`Login payload data => ${JSON.stringify(postData)}`);
    userLogger.info(__filename, `addshop payload data => ${JSON.stringify(postData)}`);
    const shop = [
        postData.shopDomain,
        postData.token,
        postData.apiKey,
    ...
    const promise = new Promise(async function (resolve, reject) {
        try {
            let data = [];
            let rows = await mysql.getSelectQueryData("SELECT * FROM qq_shop where shopDomain = ?", [postData.shopDomain], pool);
            userLogger.info(__filename, `rows => ${JSON.stringify(rows)}`);
            if (Object.keys(rows).length > 0) {
                userLogger.error(__filename, `Return null SELECT * FROM qq_shop where shopDomain = ${postData.shopDomain}`);
                // reject(`App Already installed ..!`);
                reject({ "status": "failed", "code": 0, "message": "App Already installed ..!" });
            } else {
                const query = "INSERT INTO `qq_shop`( `shopDomain`, `token`, `apiKey`, `scope`) VALUES (?,?,?,?)";
                userLogger.info(__filename, `${query} => ${JSON.stringify(shop, null, 4)}`);
                let result = await mysql.setQueryData(query, shop, pool);
                userLogger.error(__filename, `Return nresult = ${result}`);
                if (result) {
                    resolve({ "status": "success", "code": 1, "message": "New shop added ...." });
                }
                else {
                    reject({ "status": "failed", "code": 0, "message": "Error on adding new shop..." });
                    // reject(data);
                }
            }
        } catch (error) {
            reject({ "status": "failed", "code": 0, "message": "Error on adding new shop catch..." });
        }
    });
    promise.then(async function (data) {
        userLogger.info(__filename, `User Data 111  ${JSON.stringify(data)}`);
        res.status(200).json({ "status": 1, "message": "Shop added success...! ", "response": data });
    }).catch(async function (error) {
        userLogger.info(__filename, `login error ${JSON.stringify(error)}`);
        res.status(200).json({ "status": 0, "message": error });
    });
    ...
    const promise = new Promise(async function (resolve, reject) {
        try {
            if (postData?.password == postData?.confirmPassword) {
                ...
                let updateQuery = "UPDATE qq_user SET `password`= ?, `resetPassword`=uuid()   WHERE   resetPassword= ?";
                const params = [newPassword, postData?.resetPassword]
                userLogger.info(__filename, `Change Password Query  ${updateQuery} ${params}`);
                let result = await mysql.setQueryData(updateQuery, params, pool);
                console.log("data", result);
                if (result?.affectedRows > 0) {
                    data.response = { "status": "success", "code": 1, "message": "password changed.." };
                    resolve(data);
                }
                else {
                    data.response = { "status": "failed", "code": 0, "message": "Link is expired please generate new link." };
                    reject(data);
                }

            } else {
                data.response = { "status": "failed", "code": 0, "message": "Password not match..." };
                reject(data);
            }
        } catch (error) {
            reject({ "status": "failed", "code": 0, "message": "Error on changeing password ..." });
        }
    });
    promise.then(async function (data) {
        userLogger.info(__filename, `User Data 111  ${JSON.stringify(data)}`);
        res.status(200).json({ "status": 1, "message": "success...! ", "response": data });
    }).catch(async function (error) {
        userLogger.info(__filename, `login error ${JSON.stringify(error)}`);
...
router.post('/token', (req, res) => {
    ...
    if (token) {
        ...
        jwt.verify(token, config.refreshTokenSecret, function (err, decoded) {
            if (err) {
                return res.status(401).json({ "error": true, "message": 'Usnauthorized access.' });
            }
            req.decoded = decoded;
        });
    }
...
router.post('/shoplist', (req, res) => {
    // all secured routes goes here
    const postData = req.body;
    userLogger.info(__filename, 'Shoplist payload data => ' + { data: `${JSON.stringify(postData)}` });
    const order = postData.order == "DESC" ? "DESC" : "ASC";
    const query = `SELECT * FROM \`qq_shop\` WHERE status = ? and isDeleted = ?  ORDER BY ? ${order}  limit ? , ?`;
    const param = [postData.status, postData.isDeleted, postData.byId, postData.offset, postData.limit];
    const pool = null;
    const promise = new Promise(async function (resolve, reject) {
        try {
    ...
    promise.then(async function (data) {
        userLogger.info(__filename, 'Shop Data Data ' + { data: `${JSON.stringify(data)}` });
        res.status(200).json(data);
    }).catch(async function (error) {
        userLogger.info(__filename, 'Shop data error' + { data: `${JSON.stringify(error)}` });
        res.status(404).send('Invalid request')
    });
    //res.send('I am secured...');
})
